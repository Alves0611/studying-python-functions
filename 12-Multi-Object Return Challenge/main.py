...
def split_lists(prices, sizes, factor=0.1):
    if len(prices) == len(sizes):
        i = int((1 - factor) * len(prices))
        train_prices = prices[:i]
        test_prices = prices[i:]
        test_sizes = sizes[i:]
        train_sizes = sizes[:i]
        return train_prices, test_prices, train_sizes, test_sizes
    else:
        print('As listas de precos e tamanhos dos imoveis não tem a mesma quantidade de itens')
        return None